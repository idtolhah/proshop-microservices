...
const ProfileScreen = ({ location, history }) => {
  ...
  const getMyOrdersByStatus = (status) => {
    setStatus(status)
  ...
  const MyOrdersTable = ({orders}) => {
    if(orders.length <= 0){
      return <Message variant='warning'>No Orders</Message>;
    }else{
      return (
    ...
    <Row>
      ...
      <Col md={9}>
        ...
        {loadingOrders ? (
          ...
          <Tabs
            id="controlled-tab-orders"
            defaultActiveKey="created"
            activeKey={status}
            onSelect={(k) => getMyOrdersByStatus(k)}>
            <Tab eventKey="created" title="New">
              <MyOrdersTable orders={orders} />
            </Tab>
            {/* <Tab eventKey="paid" title="Paid">
              Paid orders, wait for processing
            </Tab> */}
            <Tab eventKey="paid" title="Processed">
              <MyOrdersTable orders={orders} />
            </Tab>

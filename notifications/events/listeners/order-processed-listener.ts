...
import { Listener, OrderProcessedEvent, Subjects } from '@ta-shop-simple/common';
...
export class OrderProcessedListener extends Listener<OrderProcessedEvent> {
  ...
  async onMessage(data: OrderProcessedEvent['data'], msg: Message) {
    ...
    const notification = new Notification({
      content: `Pesanaan Anda sedang diproses oleh penjual. Order ID: ${data.id}.`,
      userId:  data.userId,
      link: '/order/' + data.id,
    });
    ...
    //   body: `Pesanaan Anda sedang diproses oleh penjual. Order ID: ${data.id}.`,
    // }
